...
knit: (function(input_file, encoding) {
  out_dir <- 'knitr_output';
  rmarkdown::render(input_file,
 encoding=encoding,
 output_file=file.path(dirname(input_file), out_dir, 'OP_analysis.html'))})
output: 
  html_document:
    toc: true
    toc_float: true
...
#Applying the function above to acquire header names and concatenate. 
mouse_names <- xl_header1(ko_path,"E1:U1",3)
stat_names <- xl_header1(ko_path,"B2:D2",3)
final_header <- append(stat_names,mouse_names)

final_header <- final_header %>% 
  str_replace("^\\.+", NA_character_)

last <- tail(final_header,1)

final_header <- append(final_header,last)  

#replacing all NA values from empty cells with mouse names to preserve duplicates for "Left" and "Right"  
final_header <- final_header %>% 
  tibble() %>% 
...
ko_sheets <- xl_concat(ko_path, c(1,2),"B2:V1822", final_header)
```
