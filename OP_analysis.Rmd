...
mouse_grapher <- function(df, intensity, ms_code, time_window = 500, span = 7, threshold = 0.65, result = "both") {
  ...
  #plot the data and show local extrema
   mouse_plot <- df %>% 
    ...
    suppressWarnings(stat_peaks(aes(shape = eye, color = eye, size = df_source),
    ...
    suppressWarnings(stat_peaks(aes(color = eye, shape = eye),
               ...
               alpha = 0.7))+
    scale_size_manual(values = c(1.5, 1.51)) +
    facet_grid(eye ~ df_source, scales = "fixed", as.table = TRUE) +
    ggtitle(ms_code)+ 
    theme_bw() 
  
  #collect the extrema values in a table   
  OP_max <- layer_data(mouse_plot, i = 2L) %>% 
    select(x, y, shape, size) %>% 
...
#   for (j in mice) {
#     cat("  \n###", "**Mouse: **", j, "  \n")
#     mouse_grapher(wt_ko_sheets, intensity = i , ms_code = j , span = 9, threshold = 0.62, result = "plot")
#     cat("\n")
#     mouse_grapher(wt_ko_sheets, intensity = i , ms_code = j , span = 9, threshold = 0.62, result = "table")
#   }
# }
```


## Practice paged-tables

```{r results='asis'}
mouse_grapher(wt_ko_sheets, intensity = -2.12 , ms_code = "051" , span = 9, threshold = 0.62, result = "plot")
mouse_grapher(wt_ko_sheets, intensity = -2.12 , ms_code = "051" , span = 9, threshold = 0.62, result = "table")
```
